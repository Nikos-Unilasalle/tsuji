...
/**
 * Terminal node for the 2D Render window (see View2DWindow.tsx) — the texture
 ...
 *
 * Passes its input straight through as its own output so a 2D Render node can
 * sit mid-chain (e.g. feed a Plane's material *and* the 2D Render from the
...
/** First `view2d`-type node in the graph — mirrors findRenderNodeId. Multiple 2D Render nodes: first one wins, arbitrary order, not an error. */
