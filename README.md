...
| **Move Input** | `io/move-input` | **Un schéma de contrôle complet en un nœud.** Remplace la chaîne habituelle — cinq Keyboard + deux Action Map + un Compose Vector — par un seul nœud qui sort un vecteur de déplacement prêt à brancher sur un Character. Dispositions `ZQSD`, `WASD`, `Flèches`, combinées, ou touches personnalisées ; stick de manette fondu dans le même vecteur (le plus appuyé gagne, pas d'addition) ; sorties `move`, `jump`, `jumpPressed`, `sprint`, `magnitude`, avec lissage et zone morte. Avant est en −Z, comme partout ailleurs dans le moteur. |
