...
| **Contour Scan** | `modifier/contour-scan` | Découpe procédurale en anneaux par raycast radial BVH et morphing ondulatoire avec renflement entre deux formes (inspiré de Makio64 *venus.js*). Sorties maillage (rubans, lignes, tubes) et courbes. |
...
| **Rigid Body** | `physics/rigid-body` | Confie une géométrie au monde et la déplace selon le solveur. **`Split: per-child` par défaut** : chaque maillage — et **chaque instance** d'un `InstancedMesh` — devient son propre corps, donc vingt caisses dans un Merge ou cent boîtes dans un Array se simulent avec **un seul nœud**. `whole` pour le cas inverse (un châssis fait de plusieurs maillages doit être un corps unique). Types `dynamic` / `fixed` / `kinematic`, formes `auto` (enveloppe convexe pour un corps dynamique, maillage de triangles pour un corps fixe), `box`, `sphere`, `capsule`, `hull`, `trimesh`. Masse, frottement, rebond, amortissements, échelle de gravité, CCD. **L'échelle de l'objet est intégrée au collider** — un corps Rapier ne porte que position et rotation. |
| **Vehicle** | `physics/vehicle` | Voiture à quatre roues sur le contrôleur **raycast** de Rapier : un rayon part de chaque ancrage de roue et applique suspension, motricité et frottement latéral au châssis. C'est ainsi que procèdent presque tous les jeux de conduite — des roues simulées vibrent à l'arrêt, accrochent sur les jointures de triangles et demandent un réglage de solveur que personne ne veut faire. Ici le châssis est **un seul corps rigide** et les roues sont du calcul. Traction arrière, direction avant, freins, suspension complète, et **hauteur du centre de gravité** (le réglage qui décide si la voiture fonctionne : trop haut, l'accélération la cabre sur deux roues qui n'ont alors plus d'adhérence). Sortie `wheels` : une liste de matrices monde prêtes à recevoir n'importe quelle géométrie, angle de braquage et rotation de roulement inclus. |
...
| **Move Input** | `io/move-input` | **Un schéma de contrôle complet en un nœud.** Remplace la chaîne habituelle — cinq Keyboard + deux Action Map + un Compose Vector — par un seul nœud qui sort un vecteur de déplacement prêt à brancher sur un Character. Dispositions `ZQSD`, `WASD`, `Flèches`, combinées, ou touches personnalisées ; stick de manette fondu dans le même vecteur (le plus appuyé gagne, pas d'addition) ; sorties `move`, `forward`, `x`, `jump`, `jumpPressed`, `sprint`, `magnitude`, avec lissage et zone morte. Avant est en −Z, comme partout ailleurs dans le moteur. |
